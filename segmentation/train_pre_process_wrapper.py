...
def plan_train(argv): 
    # Plan, pre-process and then train the network
    parser = argparse.ArgumentParser()
    parser.add_argument('task', type=str)
    parser.add_argument('data', type=str)
    ...
    with CustomizedCarbonTracker(prepdir / 'carbontracker', enabled=args.carbontracker):
        ...
        # Run training: after the preprocessing is done we move on to the training section
        ...
        if args.validation_only:
            ...
            cmd.append('-p') # for loading the pretrained model and continue from
            cmd.append(PLANS)  # the plan identifire of the pretrained model
        else:
        ...
        if args.pretrained_weights is not None:
            if any(outdir.glob("*.model")):
                # In case that the .model file exists in the result forlder we use it a pretrained weights
...
def ensemble(argv):
    ...
    cmd = ['nnUNet_ensemble','-f', *[str(f) for f in output_dirs],'-o', str(output_dir)]
